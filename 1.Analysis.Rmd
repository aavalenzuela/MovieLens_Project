...
movielens %>%
  ...
  ggplot(aes(x=as.Date(as.POSIXct(timestamp, origin = "1970-01-01")),y=n)) + 
  geom_line() + 
  #scale_x_log10() +
  ggtitle("Timestampof the rate") +
  xlab("Number of users who rate movies")
```


```{r}
edx %>% ggplot(aes(year)) + geom_histogram(bins = 10)
edx %>% ggplot(aes(as.Date(as.POSIXct(timestamp, origin = "1970-01-01")))) + geom_histogram(bins = 10)
```
