...
```{r,echo=FALSE}
rmse_results <- data.frame(model="Naive Mean-Baseline Model", RMSE = mu_rmse)
```
...
```{r}
rmse_movie_model <- RMSE(test_set$rating, predicted_ratings)
...
$$
 y_{u,i} = \mu + b_{i,r} + b_{u} + \varepsilon_{i,u}
$$

 
  $\mu$ the "true" rating for all movies and $b_{i,r}$ the average rating for movie i or the "movie effect" considering the numbers of votes that the movie i has and $b_{u}$ the average rating for user u or "user effect" and $\varepsilon_{i,u}$ independent errors sampled from the same distribution centered at 0.
...
```{r}
rmse_movie_model_reg_user <- RMSE(predicted_ratings, test_set$rating)
rmse_results <- bind_rows(rmse_results,
                          data_frame(model="Movie-Based Model Regul + User Effects Model",  
                                     RMSE = rmse_movie_model_reg_user ))
```
...
genre_avgs <- train_set %>% filter(genres != "(no genres listed)") %>% separate_rows(genres) %>%
   left_join(movie_reg_avgs, by='movieId') %>%
   left_join(user_reg_avgs, by='userId') %>%
   group_by(genres) %>%
