...
We start with the average of the movie as the nitial predictor.
```{r}
...
Fer regularization, we already know the we get better result condifering both movie and user regularization at the same time. We are going to use $\lambda$ calculated before (`r lambda`)
...
As the last step it is use the genre of the movie as a predictor. For that we are going to separete each movie in each genre as we did before with the variable edx_genres.
